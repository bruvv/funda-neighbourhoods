...
export function makeSettingsButtonHtml() {
  ...
  const configureBadgesBadge = `
    <li
      class="funda-neighbourhoods-badge funda-neighbourhoods-configure-badge fd-border-radius fd-m-right-2xs"
      style="${style}"
      title="${chrome.i18n.getMessage("configureBadges")}"
      role="button"
      tabindex="0"
      aria-label="${chrome.i18n.getMessage("configureBadges")}"
      data-test="badge-configure"
    >
      ${svg}
